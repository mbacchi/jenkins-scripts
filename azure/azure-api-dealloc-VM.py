...
def dealloc_vm():
    async_vm_stop = compute_client.virtual_machines.deallocate(group, vm_name)
    async_vm_stop.wait()
...
if __name__ == "__main__":
    ...
    if status != 'VM stopped':
        print("Error: VM not in \'VM stopped\' state, currently \'{}\' state. Exiting...".format(status))
        exit(1)
    
    print("Deallocating VM \'{}\'".format(vm_name))
    dealloc_vm()
